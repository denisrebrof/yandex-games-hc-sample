...
namespace Zenject
{
    ...
    public class ZenjectDynamicObjectInjection : MonoBehaviour
    {
        ...
        static readonly MethodInfo _contextInstallZenjectBindingMethod =
            typeof(Context).GetMethod("InstallZenjectBinding",
                BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance);

        static readonly Stack<List<ZenjectBinding>> _reusableZenjectBindingLists =
            new Stack<List<ZenjectBinding>>(new[] { new List<ZenjectBinding>() });
        ...
        void Awake()
        {
            ...
            {
                var zenjectBindingList = (_reusableZenjectBindingLists.Count != 0)
                    ? _reusableZenjectBindingLists.Pop()
                    : new List<ZenjectBinding>();
